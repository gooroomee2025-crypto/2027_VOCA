...
(function () {
  ...
  /* ---------------------------------------------------------
     ...
     세법OX/VOCAVOCA 앱과 같은 Firebase 프로젝트를 쓰되, 컬렉션을 분리해서 데이터가 섞이지 않게 함
  --------------------------------------------------------- */
  var firebaseConfig = {
  ...
  var db = null;
  var syncCode = localStorage.getItem('ssvocaSyncCode') || null;
  ...
  function pullFromCloud() {
    ...
    return db.collection('ssvocaSyncData').doc(syncCode).get().then(function (snap) {
  ...
  function pushToCloud() {
    ...
    return db.collection('ssvocaSyncData').doc(syncCode).set({
      state: state,
      updatedAt: Date.now()
  ...
  function syncOnLoad() {
    ...
    return pullFromCloud().then(function (cloudData) {
      var localUpdated = Number(localStorage.getItem('ssvocaLastUpdated') || 0);
      if (cloudData && cloudData.updatedAt > localUpdated) {
        state = Object.assign(defaultState(), cloudData.state);
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (e) { /* ignore */ }
        localStorage.setItem('ssvocaLastUpdated', String(cloudData.updatedAt));
      } else {
        return pushToCloud().then(function () {
          localStorage.setItem('ssvocaLastUpdated', String(Date.now()));
        });
      }
    });
  }
  ...
  function setupSync() {
    var input = window.prompt(
      '다른 기기와 동기화할 코드를 입력하세요.\n(같은 코드를 입력한 기기끼리만 서로 연동됩니다. 다른 사람과 같이 쓸 경우 서로 다른 코드를 정해주세요.)',
      syncCode || ''
    );
    ...
    syncCode = trimmed;
    localStorage.setItem('ssvocaSyncCode', syncCode);
