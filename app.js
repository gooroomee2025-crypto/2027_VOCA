...
(function () {
  ...
  function sortedWordsOfDay(day, prog) {
    ...
    if (wordsSortMode === 'unknown') {
      list = list.slice().sort(function (a, b) {
        var aK = prog.known.indexOf(a.id) !== -1;
        var bK = prog.known.indexOf(b.id) !== -1;
        if (aK === bK) return a.no - b.no;
        return aK ? 1 : -1;
      });
  ...
  function renderWordsListView(day, prog) {
    ...
    var knownCount = list.filter(function (w) { return prog.known.indexOf(w.id) !== -1; }).length;
    var allChecked = knownCount === list.length && list.length > 0;

    var masterCheck = h('input', {
      type: 'checkbox',
      class: 'words-master-checkbox',
      checked: allChecked ? 'checked' : null,
      onchange: function (e) {
        var checkAll = e.target.checked;
        list.forEach(function (w) { setWordKnown(w.id, checkAll); });
        saveState();
        render();
      }
    }, []);
    ...
    var toolbar = h('div', { class: 'words-list-toolbar' }, [
      ...
      h('div', { class: 'words-select-summary' }, [
        h('span', { class: 'words-select-count' }, [String(knownCount) + '\uAC1C \uC120\uD0DD\uB428']),
        masterCheck
      ])
    ]);
    ...
    list.forEach(function (w) {
      listEl.appendChild(buildWordListRow(w, prog));
    });
  ...
  function buildWordListRow(w, prog) {
    var isFav = state.favorites.indexOf(w.id) !== -1;
    var isKnown = prog.known.indexOf(w.id) !== -1;
    ...
    var body = h('div', {
      ...
      onclick: function () { setWordKnown(w.id, !isKnown); saveState(); render(); }
    }, [
      h('div', { class: 'word-list-word' }, [w.word]),
      w.meaning ? h('div', { class: 'word-list-meaning' }, [w.meaning]) : null
    ]);

    var checkbox = h('input', {
      type: 'checkbox',
      class: 'word-list-checkbox',
      checked: isKnown ? 'checked' : null,
      onclick: function (e) { e.stopPropagation(); },
      onchange: function (e) { setWordKnown(w.id, e.target.checked); saveState(); render(); }
    }, []);

    return h('div', { class: 'word-list-item' + (isKnown ? ' checked' : '') }, [bookmarkBtn, body, checkbox]);
  }

  function setWordKnown(wordId, known) {
    ...
    if (known) {
      ...
      if (prog.known.indexOf(wordId) === -1) prog.known.push(wordId);
      if (prog.studied.length >= WORDS_PER_DAY) markStudyLogToday();
    } else {
      prog.studied = prog.studied.filter(function (id) { return id !== wordId; });
    ...
    q.choices.forEach(function (c, idx) {
      ...
      choicesWrap.appendChild(h('button', {
        ...
        onclick: function () {
          ...
          q.answered = true;
          q.selectedIdx = idx;
          if (c.correct) quizSession.score++;
          else {
